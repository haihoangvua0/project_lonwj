"""File-App 2: Calculator Casio FX-580 mode Complex"""
...
pfe.app_open(1)
pfe.stor_cmplx(1) # cửa ngõ để xài complex choice
SMART_TOKENS = [
        "sin(", "cos(", "tan(",
        "log(", "ln(", "sqrt(",
        "inte(", "d_dx(", "sums(", "muls(",
...
class Calculator_fx:
        def __init__(self):  
                ...
                self.fact_reg = "S"
                self.history = deque(maxlen=15) # Lưu trữ tuple kết quả
                self.current = ""
        ...
        def _history_item(self, expr, result):
                if isinstance(result, (pfe.sqrt, float, pfe.Decimal, pfe.Fraction, pfe.Pi, pfe.euler_num, complex, pfe.Complex)) or result < 1:
                        factors = []
                else:
                ...
                if value == "^":
                        ...
                        if pfe.complex_choice:
                                if self.finish_eval:
                                        ...
                                        self.eval_state = "eval"
                                expr = self.inputs.get()
                                if pos == 0 and len(expr) == 0: self.inputs.insert(0, "i")
        ...
        def build_extra_grid(self):  
                ...
                disabled_tokens = {"inte", "d_dx", "sums", "muls"}
                for r in range(len(grid)):  
                        for c in range(len(grid[0])):
                                txt = grid[r][c]
                                state = "disabled" if txt == "" or txt in disabled_tokens else "normal"
                                btn = tk.Button(self.extra_frame, text=txt, width=2, height=2, state=state, takefocus=0)
